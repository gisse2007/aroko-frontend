...
import CategoriaProductoForm    from "../../components/forms/CategoriaProductoForm";
import CategoriaProductoDetalle from "../../components/forms/CategoriaProductoDetalle";
...
export default function CategoriasProductos() {
  const cat = useCategoriasProductos();
  ...
  const openDetail = (row) => setModal({ type: MODAL.detail, row: cat.findById(row.id_categoria) ?? row });
  ...
  const handleCreate = async (values) => {
    try {
      ...
      closeModal();
      show("Categoría registrada exitosamente.");
    } catch (err) {
  ...
  const handleEdit = async (values) => {
    try {
      ...
      closeModal();
      show("Categoría actualizada exitosamente.");
    } catch (err) {
  ...
  const handleToggle = async (row) => {
    try {
      ...
      show("Estado de la categoría actualizado exitosamente.", "info");
    } catch (err) {
  ...
  const handleConfirm = async () => {
    ...
    if (type === CONFIRM.delete) {
      try {
        ...
        show(err.response?.data?.message || "Error al eliminar la categoría.", "error");
      }
    }
  ...
  const COLUMNS = [
    ...
    { key: "total_productos", label: "Productos activos" },
    {
      key: "estado",
      label: (
  ...
  return (
    <div className={styles.page}>
      ...
      <div className={styles.header}>
        <h2 className={styles.title}>Categorías de Productos</h2>
        <Tooltip label="Registrar una nueva categoría de productos">
      ...
      <div className={styles.filters}>
        ...
        <select className={styles.select} value={cat.filterEstado}
          onChange={(e) => cat.setFilterEstado(e.target.value)}>
      ...
      <Modal open={modal.type === MODAL.create} onClose={handleCancelCreate} title="Nueva categoría de producto">
        <CategoriaProductoForm onSubmit={handleCreate} onCancel={handleCancelCreate} submitLabel="Registrar" />
      </Modal>

      <Modal open={modal.type === MODAL.edit} onClose={handleCancelEdit} title="Editar categoría de producto">
      ...
      <Modal open={modal.type === MODAL.detail} onClose={closeModal} title="Detalle de categoría">
        <CategoriaProductoDetalle categoria={modal.row} />
      </Modal>
